...
module.exports = {
    ...
    // },
    // 'switch to calculator portfolio' : function(browser){
    //     browser
    //         .windowHandles(function(result){
    //             console.log('the window names are: ', result);
    ...
    //         });
    // },
    'click on calc' : function (browser){
        console.log('location of calc', locate.calc);
        browser
            .click(locate.calc)
    },
    'switch to calc modal' : function (browser){
        browser
            .waitForElementVisible(locate.calc_title,3000);
    },
    'move to calc modal element' : function (browser){
        browser
            .moveToElement(locate.calc_title,10,10);
    },
    'scroll down' : function (browser){
        helper(browser).scrollDown();
    },
    'verify calc modal title' : function (browser){
        browser
            .getText(locate.calc_title, function(result){
                console.log('result is:', result.value);
                this.assert.equal(result.value,"CALCULATOR");
            });
    },
    'click on calc live project' : function(browser){
        browser
            .click(locate.calc_link)
    },
